...
pub async fn merge_preprocessed_module_ids(
    ...
    let optimal_range: u64 =
        std::cmp::min((merged_module_ids.len() * 20) as u64, JS_MAX_SAFE_INTEGER);
    let power = 10u64.pow((optimal_range as f64).log10().ceil() as u32);
    ...
    for (module_ident, full_hash) in merged_module_ids.iter() {
        ...
        while used_ids.contains(&trimmed_hash) {
            trimmed_hash = hash_xxh3_hash64(trimmed_hash) % power;
        }
